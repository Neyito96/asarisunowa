...
const SERIES_PLAYLIST_TITLES = new Set([
  "一緒に新聞をめくろう！", "編集マニア", "8がけ社会", "朝日新聞社の歴史",
  "バスケ通信―クラッチタイム（バスクラ）", "#きのどう「木下君、あの動画みた？」",
  "GLOBE CAST", "新聞社員の「楽屋裏」", "アラサー会", "親モヤ",
...
export default function PlaylistEntryModeBridge() {
  ...
  useEffect(() => {
    ...
    const mountWhenReady = () => {
      ...
      if (!registerSection || !autoSection) {
        if (currentHost && !currentHost.isConnected) { currentHost = null; setHost(null); }
        return;
      }
      ...
      if (currentHost !== mount) { currentHost = mount; setHost(mount); }
      const nextSortHost = document.querySelector<HTMLElement>(".sorts");
      if (nextSortHost && currentSortHost !== nextSortHost) { currentSortHost = nextSortHost; setSortHost(nextSortHost); }
    ...
    const handleManagerJump = (event: Event) => {
      ...
      if (target?.closest(".playlistOwnerJump")) { setMode("autoExisting"); window.setTimeout(scrollPlaylistEntryIntoView, 0); }
    };
    document.addEventListener("click", handleManagerJump);
    return () => {
      observer.disconnect(); document.removeEventListener("click", handleManagerJump);
      const registerSection = document.querySelector<HTMLElement>("main .playlistSubmit[aria-labelledby='playlist-submit-title']");
      const autoSection = document.getElementById("auto-update-request");
      const legacyManagerButton = document.querySelector<HTMLButtonElement>(".playlistOwnerJump");
  ...
  useEffect(() => {
    ...
    const syncShelfUi = () => {
      ...
      const otherTab = buttons.find((button) => button.dataset.playlistShelfTab === "other") ?? buttons.find((button) => button.textContent?.includes("朝リスト"));
      if (!otherTab) return;
      otherTab.dataset.playlistShelfTab = "other";
      if (otherTab.textContent?.trim() !== "🐿️朝リスト") otherTab.textContent = "🐿️朝リスト";
      otherTab.onclick = () => setShelf("other");
      let seriesTab = tabs.querySelector<HTMLButtonElement>("button[data-playlist-shelf-tab='series']");
      if (!seriesTab) {
        seriesTab = document.createElement("button"); seriesTab.type = "button";
        seriesTab.dataset.playlistShelfTab = "series"; seriesTab.textContent = "連載"; tabs.insertBefore(seriesTab, otherTab);
      }
      seriesTab.onclick = () => { otherTab.click(); setShelf("series"); };
      ...
      else if (listenersActive && shelf === "other") otherTab.classList.add("on");
      ...
      const makerLabel = Array.from(document.querySelectorAll<HTMLElement>("main .playlistSubmit[aria-labelledby='playlist-submit-title'] form label > span"))
      ...
      const autoMakerLabel = Array.from(document.querySelectorAll<HTMLElement>("#auto-update-request .autoUpdateForm label > span"))
    ...
    const scheduleSync = () => { if (scheduled) return; scheduled = true; window.requestAnimationFrame(syncShelfUi); };
    syncShelfUi();
    const observer = new MutationObserver(scheduleSync);
    observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["class"] });
    window.addEventListener("resize", scheduleSync);
    return () => {
      observer.disconnect(); window.removeEventListener("resize", scheduleSync);
      document.querySelector<HTMLButtonElement>("button[data-playlist-shelf-tab='series']")?.remove();
      const otherTab = document.querySelector<HTMLButtonElement>("button[data-playlist-shelf-tab='other']");
      if (otherTab) { otherTab.textContent = "🐿️朝リスト"; delete otherTab.dataset.playlistShelfTab; otherTab.onclick = null; }
      document.querySelectorAll<HTMLElement>("#playlist-results article.card").forEach((card) => { card.hidden = false; });
      const omikuji = document.querySelector<HTMLElement>(".omikujiPanel");
  ...
  useEffect(() => {
    ...
    if (mode === "register") { registerSection.hidden = false; autoSection.hidden = true; return; }
    if (mode === "registerAndAuto") { registerSection.hidden = true; autoSection.hidden = true; return; }
    registerSection.hidden = true; autoSection.hidden = false;
  ...
  const jumpToManagerForm = () => { setMode("autoExisting"); window.setTimeout(scrollPlaylistEntryIntoView, 0); };

  return (
    <>
      {host?.isConnected ? createPortal(
        <div>
          <div className="playlistSubmitHead"><div>
            <p className="kicker">GROW YOUR PLAYLIST</p>
            <h3>🌱 リストを育てる！</h3>
            <p>朝日新聞ポッドキャストのエピソードを、シリーズ・出演者・テーマなど、自分の好きな切り口でまとめたプレイリストを登録できます。Spotifyなら、条件に合う新しい回を探して自動で追加することもできます。</p>
            <PlaylistEntryModeSelector value={mode} onChange={setMode} />
          </div></div>
          {mode === "registerAndAuto" && <PlaylistCombinedAutoForm />}
        </div>, host) : null}
      {sortHost?.isConnected ? createPortal(
        <button type="button" className="playlistManagerSortButton" onClick={jumpToManagerForm}
          aria-label="リストを育てるメニューへ" title="プレイリストを登録・自動更新"
          style={{ marginLeft: "auto", borderColor: "#cc4b78", color: "#0b5874", fontWeight: 800 }}>
          🌱 リストを育てる！
        </button>, sortHost) : null}
    </>
  );
}
